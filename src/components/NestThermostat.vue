    ...
    >
      <defs>
        ...
        >
            ...
            <path
              :fill="stateIconColor"
              :transform="`translate(${iconOffset.x} ${iconOffset.y}) scale(2)`"
        ...
        >
          <rect
            :width="diameter * 0.12"
            :height="diameter * 0.14"
            :x="iconOffset.offsetX(0.15)"
            :y="iconOffset.y"
            fill="transparent"
          />
          <path
            :fill="modeIconColor"
            :transform="`translate(${iconOffset.offsetX(0.15)} ${iconOffset.y}) scale(2)`"
...
export default {
  ...
  watch: {
    offsetMinutes (minutes, oldMinutes) {
      ...
      if(Math.abs(holdUntil.diff(time, 'minutes')) < 15 && oldMinutes !== 0) {
        this.$emit('update:presetMode', 'HOLD_UNTIL')
        return
      }
  ...
  computed: {
    hasHoldUntil() {
      return this.holdUntil.length > 0
    },
    isHeating () {
    ...
    iconOffset() {
      ...
      return {
        x: _this.hasHoldUntil ? _this.diameter * 0.36 : _this.diameter * 0.44,
        y: _this.hasHoldUntil ? _this.diameter * 0.83 : _this.diameter * 0.85,
        offsetX(x) {
          return _this.diameter * (0.36 + x)
        }
      }
    }
  },
  methods: {
    ...
    getActiveTrack(low, high) {
      return {
        fill: 'none',
        stroke: this.isHeating ? this.heatingColor : this.inactiveTickColor,
        'stroke-linecap': 'round',
        'stroke-width': 20,
